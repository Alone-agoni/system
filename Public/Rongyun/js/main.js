            ...
            if (list.length==0) {
                ...
                $.each(list, function(index, val) {
                    ...
                    if (val['senderUserId']==uid) {
                        ...
                        messageContent['avatar']=userInfo['face'];
                    }else{
                        messageContent['direction']='bjy-cbhla-right';
                        messageContent['avatar']=xbUserInfo['face'];
                    }
...
function rongCreateMessage(messageContent){
    ...
    var str = "";
    str = "<div class=\"message\"><div class=\"face_r\">:"+messageContent['name']+"</div><div class=\"msg_r\">"+messageContent['content']+"</div><div class=\"clear\"></div></div>";
    return str;
}
    ...
    RongIMClient.getInstance().clearUnreadCount(conversationtype,uid,{
        onSuccess:function(isClear){
            ...
            if (isClear) {
                ...
                // 获取原来的未读消息数量
                var oldCount=htmlObj.find('.flo-count').text();
...
function rongCreateFriendInfo(userInfo) {
    ...
    var str = "";
    str = "<li data-face='"+userInfo['face']+"' data-name='"+userInfo['name']+"' data-uid='"+userInfo['id']+"' class='fl-one'>"+userInfo['name']+"<span class='flo-count'>未读消息"+userInfo['count']+"</span></li>";
    return str;
}
...
function rongRecountMessage(uid,num){
    // 获取此用户在好友列表中的html对象
    var htmlObj=rongGetFriendListObj(uid);
    var countObj=htmlObj.find('.flo-count');
    // 获取原来的未读消息数量
...
function rongChangeScrollHeight(num){
    $('#cb-history').scrollTop(num);
}

$(function(){
    // 点击左侧好友列表；获取历史消息
    $('#friend-list').on('click', '.fl-one', function(event) {
        // 获取用户信息
        var uid=$(this).attr('data-uid'),
            userInfo={
                'name': $(this).attr('data-name'),
                'avatar': $(this).attr('data-face'),
            };
        // 如果已经选中；则不再获取历史消息
        if (!$(this).hasClass('flo-checked')) {
            $('#cb-history').html('');
            // 增加选中样式
            $(this).addClass('flo-checked').siblings('.fl-one').removeClass('flo-checked');
            // 设置uid
            $('#cbh-send').attr('data-uid',uid);
            $('#cbh-send').attr('data-avatar',userInfo['face']);
            // 设置聊天框title
            $('#tt-name').text(userInfo['name']);
            // 获取历史记录
    ...
    // 点击发送消息按钮
    $('#cbh-send').click(function(event) {
        // 获取消息内容和uid
        var str=$('#cbs-content').val(),
        ...
        var messageContent={
            ...
            'name':xbUserInfo['name'],
            'isStudent':xbUserInfo['is_student'],
            'content':str
        };
        var messageStr=rongCreateMessage(messageContent);
        $('#cb-history').append(messageStr);
        // 调整滚动轴到底部
        rongChangeScrollHeight(99999);
        // 发送消息
